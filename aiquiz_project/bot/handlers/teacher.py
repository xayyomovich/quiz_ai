...
async def publish_test(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        message_text = (
            ...
            f"📱 <b>QR Code attached below ⬇️</b>\n\n"
            f"💡 Students can click the link or scan the QR code to start!\n\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📋 Use <b>My Tests</b> to:\n"
            f"   • View test content\n"
            f"   • Get link again\n"
            f"   • See results & statistics"
        )
        ...
        # Reset state to MAIN_MENU and show keyboard
        await state.set_state(TeacherStates.MAIN_MENU)
...
async def create_new_test_callback(callback: CallbackQuery, state: FSMContext):
    ...
    # **FIX: Clear state data from previous test**
    await state.clear()
